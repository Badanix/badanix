...
const Profile = () => {
  ...
  const [accountSettings, setAccountSettings] = useState({
    ...
    age: userData?.data?.dob
      ? `${differenceInYears(new Date(), parseISO(userData.data.dob))} Years`
      : null,
  ...
  const [newCompany, setNewCompany] = useState({
    ...
    blood_type: userData?.data?.blood_type,
    nameOfInsuranceProvider: userData?.data?.name_insurance_provider,
    typeOfInsuranceProvider: userData?.data?.type_insurance_provider,
    policyNumber: userData?.data?.policy_number,
    insuranceCoverage: userData?.data?.insurance_coverage,
    insuranceValidityPeriod: userData?.data?.insurance_validity_period,
    exerciseRoutine: userData?.data?.exercise_routine,
    nutritionPlan: userData?.data?.nutrition_plan,
    smokingHabbits: userData?.data?.smoking_habbits,
    alcohol_consumption: userData?.data?.alcohol_consumption,
    sleep_pattern: userData?.data?.sleep_pattern,
    name_primary_physician: userData?.data?.name_primary_physician,
    existing_med_condition: userData?.data?.existing_med_condition,
    current_medication: userData?.data?.current_medication,
    allergies: userData?.data?.allergies,
    past_surgeries: userData?.data?.past_surgeries,
    chronic_illness: userData?.data?.chronic_illness,
    mental_health: userData?.data?.mental_health,
    family_health_history: userData?.data?.family_health_history,
  });
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      if (response?.data?.message) {
        ...
        const updatedUserDataFromStorage = JSON.parse(
          localStorage.getItem("user_data")
        );
        const newProfPics = updatedUserDataFromStorage?.data?.prof_pics;

        setUserImage(
          newProfPics ? `${defaultUrl}${newProfPics}` : defaultUserImage
        );
        console.log("Merged Data:", updatedUserData);
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      ...
      // API URL
      const apiUrl = APIURLS.APIURLPATIENTSCHANGEPASSWORD;
              ...
              <div className="rounded-3xl lg:mt-3 lg:w-3/12 shadow-xl ml-3 bg-white items-center mx-auto my-auto p-8 ">
                <div className="relative w-[120px] h-[120px] mx-auto ">
                  ...
                  <div>
                    ...
                    <button
                      type="button"
                      className={`mt-5 w-full text-white py-1 rounded-md ${
                        selectedFile
                          ? "bg-primary border-secondary hover:bg-secondary"
                          : "bg-secondary border-primary hover:bg-primary"
                      }`}
                      onClick={
                        selectedFile
                          ? handleImageUpload
                          : () => document.getElementById("prof_pics").click()
                      }
                    >
                ...
                {activeTab === "password" && (
                  <div className="p-4">
                    <div className=" justify-center w-full lg:w-11/12 mx-auto   items-center space-y-4">
                      ...
                      >
                        <div className="my-3">
                          <p className="text-gray-600  w-full lg:w-10/12 mx-auto">
                            Old Password
                          </p>
                          <input
                            type="password"
