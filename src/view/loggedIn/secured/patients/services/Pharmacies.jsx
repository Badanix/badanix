...
import { usePharmacyServices } from './ServicesForm';
import { FaLocationDot } from 'react-icons/fa6';
import { Link } from 'react-router-dom';
import { useEffect, useState } from 'react';
import { MdOutlineDoubleArrow, MdOutlineLocalPharmacy, MdSick,MdOutlineWorkOutline } from 'react-icons/md';
import { FaArrowLeft, FaStar } from 'react-icons/fa';
const Pharmacies = () => {
  const { isSidebarOpen, toggleSidebar } = UseSideBarMenu();
  const {pharmacyData,loading}= usePharmacyServices()
  ...
  const totalItems = pharmacyData.filter((item) => item.verified === 1).length;
  const totalPages = Math.ceil(totalItems / currentItemsPerPage);

  // Filter the verified items
  const verifiedData = pharmacyData.filter((item) => item.verified === 1);
        ...
        {loading ? (
          <div className="flex items-center justify-center h-screen">
            <div className="text-center">
              <MdOutlineLocalPharmacy size={48} className="text-secondary mb-4 mx-auto" />
              <p className="text-gray-600 font-semibold">Loading</p>
            </div>
          </div>
        ) : (
          <>
          <div className='flex justify-between space-x-3 mt-[70px] md:mt-4'>
            <Select
              options={countries}
            ...
            <div className="md:mt-5   bg-gray-50 p-4 w-full relative" style={{ borderTopLeftRadius: '20% 20px', borderTopRightRadius: '20% 20px', zIndex: 10 }}>
              <div className="mt-6 mb-[50px]">
                ...
                <p className="flex items-center text-gray-500 mt-3 ">
                  ...
                  {selectedItem.address} {selectedItem.city} {selectedItem.state} {selectedItem.country}                </p>
