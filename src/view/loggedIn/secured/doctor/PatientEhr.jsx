  ...
  const fetchEhrRecord = async () => {
    ...
    try {
      ...
      if (res.ok && data?.data?.length > 0) {
        ...
        Swal.fire('Success', 'Record fetched successfully', 'success');
      } else {
        setRecord(null);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto bg-white min-h-screen">
      ...
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-2xl font-bold text-[#14361D]">Enter Patient EHR:</h1>
      </div>
      ...
      {record && record.data?.length > 0 ? (
              ...
              {record.data.map((item, index) => (
                <tr key={index} className="hover:bg-gray-100">
                  ...
                  <td className="p-2 border">
                    {item.medication
                      ? item.medication.split(',').map((med, i) => (
                          <div key={i}>{med.trim()}</div>
                        ))
                      : 'No medication'}
                  </td>
                  <td className="p-2 border">
                    {item.lab
                      ? item.lab.split(',').map((lab, i) => <div key={i}>{lab.trim()}</div>)
                      : 'No lab result'}
                  </td>
        ...
        <p className="mt-6 text-red-600 text-center">No record found for this EHR</p>
      )}
    </div>
  );
};
