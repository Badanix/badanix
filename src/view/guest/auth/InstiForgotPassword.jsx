...
const InstiForgotPassword = () => {
  ...
  const getOTPCode = async (e) => {
    ...
    try {
      const response = await fetch(APIURLINSTITUTIONFORGOTPASSWORD, {
        method: 'POST',
        headers: {
      ...
      Swal.fire('Success', data.message || 'OTP sent to your email', 'success').then(() => {
        window.location.href = `/instireset-code?email=${encodeURIComponent(formData.email)}`;
      });
    } catch (error) {
      ...
      <main className={styles.mainContainer}>
        ...
        <section className={styles.mainRegister}>
          <div className="border-b border-gray-200 sm:max-w-[30rem] mb-4 text-primary">
            <h1 className={`text-primary dark:text-secondary font-bold ${styles.subHeading}`}>Institution Forgot Password</h1>
          </div>
...
export default InstiForgotPassword;
