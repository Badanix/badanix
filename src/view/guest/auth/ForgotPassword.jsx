...
const ForgotPassword = () => {
  ...
  const getOTPCode = async (e) => {
    ...
    try {
      const response = await fetch(APIURLDOCTORFORGOTPASSWORD, {
        method: 'POST',
        headers: {
      ...
      <main className={styles.mainContainer}>
        ...
        <section className={styles.mainRegister}>
          <div className="border-b border-gray-200 sm:max-w-[30rem] mb-4 text-primary">
            <h1 className={`text-primary dark:text-secondary font-bold ${styles.subHeading}`}>Doctor Forgot Password</h1>
          </div>
